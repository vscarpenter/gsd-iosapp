...
- **Dependency-cleanup-on-delete is a domain *policy* implemented in the store layer (→ Phase 1 layering refinement).** `GRDBTaskRepository.delete` scrubs the deleted id from other tasks' `dependencies` (§6.8) — correct and now stamps `updatedAt` on the affected rows via an injected clock. But the *rule* ("dangling dependency ids must be removed") arguably belongs in `GSDModel`/the use-case layer, with the store just persisting the result. When Phase 1 introduces the use-case/store layer that stamps `updatedAt` on primary mutations, consider relocating this policy there so `GSDStore` stays purely persistence. Not a correctness issue today.
