...
- **✅ Protocol CONFIRMED live via Probe P1 (2026-06-03).** Captured against `api.vinny.io` (fixture `GSDKit/Tests/GSDSyncTests/Fixtures/realtime_connect.txt`; regression tests in `RealtimeEventTests`): `GET /api/realtime` → `id:<clientId>` / `event:PB_CONNECT` / `data:{"clientId":…}` (NO space after the colon); `POST /api/realtime {"clientId":…,"subscriptions":["tasks"]}` with `Authorization` → **HTTP 204**; collection events are `event:tasks` + `data:{"record":{…§7.1…},"action":"create|update|delete"}`. **A DELETE event carries the FULL record incl. `task_id`** (so realtime deletes apply directly — the earlier "degrade to cadence reconcile" assumption was conservative and is not needed). Absent number fields (`notify_before`/`estimated_minutes`) come back as `0`, not null. The implementation (`SSEParser`/`RealtimeEvent`/`PocketBaseRealtime`/`applyRealtime`) matches this byte-for-byte. *(Two-device end-to-end behavior — echo-filter across devices, reconnect-on-foreground, network-regained — is still exercised at the A74 live gate.)*
