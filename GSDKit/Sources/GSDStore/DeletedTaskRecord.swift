...
/// GRDB row for a TRASHED task — the full §5.1 column set (identical to `tasks`) plus
/// `deletedAt`. Lives in a SEPARATE `deletedTasks` table so trashed rows are excluded from
/// the matrix/smart-view queries by construction, exactly as `ArchivedTaskRecord` is. The
/// task-column mapping is delegated to `TaskRecord` to avoid duplicating 24 fields.
struct DeletedTaskRecord: Codable, FetchableRecord, PersistableRecord {
    static let databaseTableName = "deletedTasks"
...
extension DeletedTaskRecord {
    /// Build from a domain task + the deletion timestamp. Reuses `TaskRecord` for the
    /// task-column JSON encoding so the two records stay in lockstep.
    init(_ task: Task, deletedAt: Date) throws {
    ...
    /// Reconstruct the domain task (drops `deletedAt`, which is trash-only metadata).
